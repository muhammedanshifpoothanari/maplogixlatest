...
export function BidChatForOwner({ onClose }:any) {
  ...
  return (
    <div className="relative z-10 flex flex-col w-full p-4  bg-white rounded-lg shadow">
...
<div className=" flex flex-col space-y-4 overflow-y-auto h-[260px]   p-2 border-b-2 border-gray-200">
  ...
  {msg.text?.startsWith('$_%') ?'bidAmount is:'+ msg.text.slice(3) : msg.text}
      ...
      <div className="flex items-center justify-center p-2 space-x-2">
        <Input className="flex-1" placeholder="Enter the bid amount" onChange={e => setinputMessage('$_%'+e.target.value)}/>
        <Button className="flex-shrink-0 w-[80%] hover:bg-blue-500 hover:text-white" variant={'secondary'} onClick={submitBid}>Bid Approve</Button>
      </div>
      :null}
    </div>
  );
